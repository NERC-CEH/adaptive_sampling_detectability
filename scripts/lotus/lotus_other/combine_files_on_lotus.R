...
for(v in c(1,4)) { #1:asv){
  ...
  ## index file
  community_folder = paste0(dirs$commpath, community_version, simulation_run_name, "/", community_version, sprintf("community_%i_%i_sim/", n_communities, max(n_species)))
  ...
  write.csv(out, file = paste0(dirs$commpath, community_version, simulation_run_name, 
