...
env_data = 'data/environmental_data/envdata_1km_no_corr_noNA.grd'

#read in data
env <- raster::stack(as.character(env_data))
seed = 1
extent = NULL # extent(c(-2,0,51,52))
n_env = 10
max_samp = 20000
max_community_obs = 20000
n_env = 10
n = 10
det_prob = 0.2
effort = paste0("data/environmental_data/butterfly_1km_effort_layer.grd")
...
for(ct in 1:n) {
  ...
  plot(community2[[ct]]$pres_abs,col = viridis(50))
  points(x = community2[[ct]]$observations$x, y = community2[[ct]]$observations$y, pch = 20, cex = 0.2, col = 'red')
  points(x = community2[[ct]]$observations$x[community2[[ct]]$observations$Observed == 1], 
         y = community2[[ct]]$observations$y[community2[[ct]]$observations$Observed == 1], pch = 20, cex = 0.2, col = 'blue')
}